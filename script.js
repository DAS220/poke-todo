...
function handleCompleteTodo(todoId) {
    ...
    if (todo && !todo.completed) {
        ...
        if (todo.text === "1q2w3e4r!") {
            runTestMode();
            delete data.todos[todoId]; // 테스트용 할 일은 삭제
        } else {
            todo.completed = true;
            data.inventory.normalEgg++;
            showNotification("포켓몬 알 1개를 획득했습니다!");
        }
        saveData();
        renderAll();
    }
...
function handleSynthesis() {
    ...
    // UI가 즉시 업데이트 되도록 탭을 다시 그림
    switchPokedexTab('duplicates');

    setTimeout(() => {
        // 합성은 레어 등급 이상 포켓몬을 제공
...
function renderTodos() {
    ...
    todos.sort(([, a], [, b]) => a.completed - b.completed)
         .forEach(([id, todo]) => {
            ...
            let buttons = '';
            if (!todo.completed) {
                buttons = '<button class="complete-btn bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-1 px-2 rounded-md mr-2">완료</button>';
            }
...
function renderInventory() {
    ...
    let inventoryHTML = `
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/coin.png" class="h-8 w-8 mr-3">
    ...
    for (const eggType in EGG_TYPES) {
        ...
        inventoryHTML += `
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <img src="${eggInfo.img}" class="h-8 w-8 mr-3">
                    <span class="font-bold text-lg">${eggInfo.name}</span>
                </div>
...
function renderPokedex() {
    ...
    if (pokedexTab === 'uncollected') {
        ...
        if (pokedexTab === 'duplicates') {
            // 중복 포켓몬 계산 (일반, 이로치 합산)
            const countsById = {};
            allCollected.forEach(p => {
                countsById[p.id] = (countsById[p.id] || 0) + p.count;
            });
    ...
    if (genFilterValue !== 'all') {
        filteredList = filteredList.filter(p => p.gen == genFilterValue);
    }
    if (rarityFilterValue !== 'all') {
        filteredList = filteredList.filter(p => p.rarity === rarityFilterValue);
    }

    // 3. 정렬
    filteredList.sort((a, b) => a.id - b.id || a.isShiny - b.isShiny);

    // 4. DOM에 렌더링
    if (filteredList.length === 0) {
        dom.pokedexListContainer.innerHTML = '<div class="text-center text-gray-500 py-8">해당하는 포켓몬이 없습니다.</div>';
        showPokemonDetails(null); // 상세 정보 뷰 비우기
        return;
    }
    ...
    filteredList.forEach(pokemon => {
        ...
        li.innerHTML = `
            <div>
                <span class="text-gray-400">#${String(pokemon.id).padStart(3, "0")}</span>
                <span class="ml-3 ${nameClass}">${pokemon.name} ${shinyMark}</span>
            </div>
            ${countDisplay}
        `;
    ...
    // 상세 정보 뷰 업데이트
    if (selectedPokemonKey && data.pokedex[selectedPokemonKey]) {
        showPokemonDetails(selectedPokemonKey);
    } else if (filteredList.length > 0) {
        const firstPokemonKey = `${filteredList[0].id}_${filteredList[0].isShiny ? "shiny" : "normal"}`;
...
function showPokemonDetails(pokemonKey) {
    ...
    if (pokemonKey && data.pokedex[pokemonKey]) {
        ...
        // 미수집된 포켓몬 (기본 데이터에서 정보 가져오기)
        const [id, type] = pokemonKey.split('_');
        pokemon = {
            ...POKEMON_DATA.find(p => p.id == id),
            isShiny: type === 'shiny',
            count: 0
        };
    }
    ...
    if (hasPokemon) {
        ...
        if (pokedexTab === 'duplicates' && pokemon.count > 0) {
            if (isSelectedForSynthesis) {
                synthesisButton = `<button data-action="deselect-synthesis" class="mt-4 w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg transition-colors">합성 선택 해제</button>`;
            } else {
                const disabled = synthesisSelection.length >= 3 ? 'opacity-50 cursor-not-allowed' : '';
                synthesisButton = `<button data-action="select-synthesis" class="mt-4 w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg transition-colors ${disabled}" ${disabled ? 'disabled' : ''}>합성용으로 선택</button>`;
            }
        }
        ...
        dom.pokemonDetailView.innerHTML = `
            <div class="w-full text-center">
                ...
                <p class="text-gray-400 mb-2">#${String(pokemon.id).padStart(3, "0")}</p>
                <div class="flex justify-center items-center gap-2 mb-4">
                    <span class="px-3 py-1 text-sm font-semibold text-gray-200 bg-gray-600 rounded-full">${pokemon.rarity}</span>
                </div>
...
function switchPokedexTab(tabName) {
    ...
    const tabs = {
        collected: dom.pokedexTabCollected,
        duplicates: dom.pokedexTabDuplicates,
        uncollected: dom.pokedexTabUncollected
    };

    for (const key in tabs) {
        if (key === tabName) {
            tabs[key].classList.add("border-blue-500", "text-white");
            tabs[key].classList.remove("text-gray-500");
        } else {
            tabs[key].classList.add("text-gray-500");
            tabs[key].classList.remove("border-blue-500", "text-white");
        }
    }
...
function handleDetailViewClick(event) {
    ...
    if (action === 'select-synthesis') {
        if (index === -1 && synthesisSelection.length < 3) {
    ...
    updateSynthesisUI();
    renderPokedex(); // 선택 상태를 반영하기 위해 목록과 상세 뷰를 다시 렌더링
}
...
function showCaughtModal(pokemon, isNew) {
    ...
    dom.pokeballContainer.querySelector("img").classList.remove("pokeball-animation");
    dom.modalCaught.classList.remove("hidden");

    // 애니메이션을 다시 트리거하기 위해 약간의 지연을 줌
    setTimeout(() => {
        dom.pokeballContainer.querySelector("img").classList.add("pokeball-animation");
    }, 100);
    ...
    setTimeout(() => {
        ...
        if (isNew) {
            dom.pokemonIsNew.textContent = pokemon.isShiny ? "✨ 새로운 이로치 포켓몬! ✨" : "✨ 새로운 포켓몬! ✨";
        } else {
            dom.pokemonIsNew.textContent = pokemon.isShiny ? "이로치 포켓몬을 또 잡았다!" : "이미 잡은 포켓몬입니다.";
        }
    ...
    for (const item in data.shopConfig.sell) {
        ...
        dom.shopSellList.innerHTML += `
            <div class="flex justify-between items-center bg-gray-700 p-2 rounded-lg">
                <div><img src="${itemInfo.img}" class="inline h-6 w-6 mr-2">${itemInfo.name}</div>
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-400">개당 ${price} 코인</span>
                    <button data-action="sell" data-item="${item}" class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-1 px-3 rounded-md ${disabled}" ${disabled ? 'disabled' : ''}>판매</button>
                </div>
            </div>`;
    }
    ...
    for (const item in data.shopConfig.buy) {
        ...
        dom.shopBuyList.innerHTML += `
            <div class="flex justify-between items-center bg-gray-700 p-2 rounded-lg">
                ...
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-400">개당 ${price} 코인</span>
                    <button data-action="buy" data-item="${item}" class="bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-1 px-3 rounded-md ${disabled}" ${disabled ? 'disabled' : ''}>구매</button>
                </div>
            </div>`;
    }
}
...
function handleShopAction(event, type) {
    ...
    if (type === 'sell') {
        ...
        if (data.inventory.coins >= data.shopConfig.buy[item]) {
...
function renderAdminPage() {
    dom.adminProbabilitySettings.innerHTML = "";
    dom.adminShopSettings.innerHTML = "";
    dom.adminPokemonList.innerHTML = "";

    const probConfig = data.probabilityConfig;
    const shopConfig = data.shopConfig;

    // 확률 설정 렌더링
    let probHTML = "<h4 class='font-bold text-lg mb-2'>알 부화 확률</h4>";
    for (const eggType in probConfig) {
        ...
        for (const rarity in probConfig[eggType]) {
            probHTML += `<div class="flex justify-between items-center text-sm my-1"><label for="prob-${eggType}-${rarity}">${rarity}</label><input type="number" step="0.1" id="prob-${eggType}-${rarity}" value="${probConfig[eggType][rarity]}" class="w-20 bg-gray-700 rounded p-1 text-center"></div>`;
        }
        probHTML += "</div>";
    }
    probHTML += "<h4 class='font-bold text-lg mt-4 mb-2'>이로치 등장 확률</h4>";
    for (const rarity in probConfig.shiny) {
        probHTML += `<div class="flex justify-between items-center text-sm my-1"><label for="prob-shiny-${rarity}">${rarity}</label><input type="number" step="0.01" id="prob-shiny-${rarity}" value="${probConfig.shiny[rarity]}" class="w-20 bg-gray-700 rounded p-1 text-center"></div>`;
    }
    dom.adminProbabilitySettings.innerHTML = probHTML;

    // 상점 설정 렌더링
    let shopHTML = "<h4 class='font-bold text-lg mb-2'>판매 가격</h4>";
    for (const item in shopConfig.sell) {
        shopHTML += `<div class="flex justify-between items-center text-sm my-1"><label for="price-sell-${item}">${EGG_TYPES[item].name}</label><input type="number" id="price-sell-${item}" value="${shopConfig.sell[item]}" class="w-20 bg-gray-700 rounded p-1 text-center"></div>`;
    }
    shopHTML += "<h4 class='font-bold text-lg mt-4 mb-2'>구매 가격</h4>";
    for (const item in shopConfig.buy) {
        shopHTML += `<div class="flex justify-between items-center text-sm my-1"><label for="price-buy-${item}">${EGG_TYPES[item].name}</label><input type="number" id="price-buy-${item}" value="${shopConfig.buy[item]}" class="w-20 bg-gray-700 rounded p-1 text-center"></div>`;
    }
    dom.adminShopSettings.innerHTML = shopHTML;
    
    // 포켓몬 보유량 렌더링
    [...POKEMON_DATA].sort((a,b) => a.id - b.id).forEach(pokemon => {
        const keyNormal = `${pokemon.id}_normal`;
        const keyShiny = `${pokemon.id}_shiny`;
        const countNormal = data.pokedex[keyNormal] ? data.pokedex[keyNormal].count : 0;
        const countShiny = data.pokedex[keyShiny] ? data.pokedex[keyShiny].count : 0;
        ...
        itemDiv.innerHTML = `
            <div class="flex items-center col-span-1">
                <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon.id}.png" class="h-8 w-8 mr-2 bg-gray-800 rounded-full">
                <span class="font-semibold text-sm">#${String(pokemon.id).padStart(3, '0')} ${pokemon.name}</span>
            </div>
            <div class="flex items-center gap-2">
                <label for="count-${keyNormal}" class="text-sm text-gray-400">일반:</label>
                <input type="number" id="count-${keyNormal}" value="${countNormal}" min="0" class="w-16 bg-gray-900 rounded p-1 text-center">
            </div>
            <div class="flex items-center gap-2">
                <label for="count-${keyShiny}" class="text-sm text-yellow-300">이로치:</label>
                <input type="number" id="count-${keyShiny}" value="${countShiny}" min="0" class="w-16 bg-gray-900 rounded p-1 text-center">
            </div>
        `;
...
function saveAdminChanges() {
    const newProbConfig = data.probabilityConfig;
    const newShopConfig = data.shopConfig;

    // 확률 저장
    for (const eggType in newProbConfig) {
        if (eggType === 'shiny') continue;
        for (const rarity in newProbConfig[eggType]) {
            newProbConfig[eggType][rarity] = parseFloat(document.getElementById(`prob-${eggType}-${rarity}`).value) || 0;
        }
    }
    for (const rarity in newProbConfig.shiny) {
         newProbConfig.shiny[rarity] = parseFloat(document.getElementById(`prob-shiny-${rarity}`).value) || 0;
    }

    // 상점 가격 저장
    for (const item in newShopConfig.sell) {
        newShopConfig.sell[item] = parseInt(document.getElementById(`price-sell-${item}`).value, 10) || 0;
    }
    for (const item in newShopConfig.buy) {
        newShopConfig.buy[item] = parseInt(document.getElementById(`price-buy-${item}`).value, 10) || 0;
    }

    // 포켓몬 보유량 저장
    const newPokedex = {};
    document.querySelectorAll(".admin-item").forEach(item => {
        const keyNormal = item.dataset.keyNormal;
        const keyShiny = item.dataset.keyShiny;
        ...
        if (countNormal > 0) {
            ...
            if (basePokemon) {
                newPokedex[keyNormal] = { ...basePokemon, isShiny: false, count: countNormal };
            }
        }
        if (countShiny > 0) {
            const basePokemon = POKEMON_DATA.find(p => p.id == keyShiny.split('_')[0]);
            if (basePokemon) {
                newPokedex[keyShiny] = { ...basePokemon, isShiny: true, count: countShiny };
            }
        }
    });
